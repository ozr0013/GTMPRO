...
When the playbook gained rules since your last action (rule-* keys at the end of the list), prefer
citing those newest rules — they encode the freshest lessons. Among older rules, prefer ones with a
strong measured track record; if you cite a rule whose track record is poor, say why this case is different.
Predicted counts are non-negative and the low bound must not exceed the high bound. If recent
calibration is poor, widen your ranges rather than repeating the same miss.
Propose a dm_reply ONLY for a thread id listed under "Open DM threads"; when none are listed, propose posts instead.
...
FIRST, before anything else: every entry in "humanRejections_MUST_ADDRESS" must produce an ADDED
rule that would have prevented that exact draft, with sourceType "rejection" (or "edit") and the
entry's proposalId in that rule's evidenceRefs. It must be an add, not an amend — amended rules keep
their original evidence, so an amend can never mark the rejection addressed. A human took the
trouble to say why — that outranks every metric in this digest, even when the outcome reports look
more interesting.
